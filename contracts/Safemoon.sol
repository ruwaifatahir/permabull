...
interface ISafeSwapTradeRouter {
    ...
    function swapExactTokensForETHAndFeeAmount(
        Trade calldata trade
    ) external payable;

    function swapTokensForExactETHAndFeeAmount(
        Trade calldata trade
    ) external payable;
    ...
    function swapTokensForExactTokensWithFeeAmount(
        Trade calldata trade
    ) external payable;
    ...
    OwnableUpgradeable
{
        ...
        isRouter(_msgSender())
    {
        ...
        // If contract token balance is greater or equal to max transaction
        // amount then reset the contract token balance to max transaction amount
        // IMPORTANT thing to note is that contractTokenBalance is just a local variable
        // not the actual contract's token balance
        ...
        // If contract's token balance is greater or equal to amount required
        // to initiate the collectBNB() function then reset the contract's token balance
        // variable to amount required to initiate collectBNB() function and
        // then call collectBNB() function
    ...
    function swapTokensForBnb(uint256 tokenAmount) p rivate {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        ISafemoon(address(this)).transferFrom(
            msg.sender,
            address(this),
            swapeedToken
        );
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {
        ...
        tradeRouter.swapExactETHForTokensWithFeeAmount{
            value: bnbAmount + feeAmount
        }(trade, feeAmount);
    }
    ...
    function addListIgnoreCollectBNBOnTransferAddresses(
    ...
    function removeListIgnoreCollectBNBOnTransferAddresses(
        address[] calldata _addresses
    ) external onlyOwner {
        uint256 len = _addresses.length;
        for (uint256 i = 0; i < len; i++) {
    ...
    function _isInCollectBNBWhitelist(
        address _addr
    ...
    function mint(
        address user,
        uint256 amount
    ) public whenNotPaused onlyWhitelistMint {
        if (msg.sender != owner()) {
            require(
                amount <= _maxTxAmount,
                "Transfer amount exceeds the maxTxAmount."
